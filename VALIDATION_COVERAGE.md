...
| eVDGA                                  | Yes             | Yes              | Fixture-backed local    | Yes         | DiGA catalog import, canonical order finalization, immutable document issuance, and DeviceRequest-based eVDGA bundle rendering are covered by workflow tests. Rendered bundles now emit a schema-backed eVDGA oracle plan and pass local FHIR bundle checks through the shared validator runner shape; official executable archive sweeps are still pending. |
...
- `eVDGA`
  - workflow-backed local coverage exercises:
    - DiGA catalog import and order finalization
    - emitted eVDGA bundle rendering with a concrete oracle plan
    - shared local FHIR checks for `Bundle`, `Composition`, `Coverage`, and `DeviceRequest`
...
4. Extend eVDGA from local FHIR plan execution to pinned official executable validator and example-archive sweeps.

      ...
      Effect.gen(function* () {
        const result = yield* provideTestConfect(
          Effect.gen(function* () {
            ...
            const { coverageId, organizationId, practitionerId } =
              yield* test.run(
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(
          result.workspaceBeforePromotion.draftWorkspace.schemaVersion,
        ).toBe(2);
        ...
        expect(result.currentPlan.plan._id).toBe(
          result.persistedPlans.secondId,
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.currentPlan.entries[0]?.entrySource).toBe(
          "imported-plan",
        );
      ...
      Effect.gen(function* () {
        const result = yield* provideTestConfect(
          Effect.gen(function* () {
            ...
            yield* test.mutation(
              refs.public.catalog.importHeilmittelCatalogRefs,
              {
                entries: [
                  {
                    blankoEligible: true,
            ...
            const orderView = yield* test.query(
              refs.public.heilmittel.getOrder,
              {
                heilmittelOrderId: order.heilmittelOrderId,
              },
            );

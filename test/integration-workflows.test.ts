...
describe("integration eeb workflows", () => {
  it("should receive a verified eEB inbox item, match the patient, and report quarter card-read status", async () => {
  ...
  it("should block eEB adoption when the quarter has no prior card read", async () => {
    const result = await runWithTestConfect(
      Effect.gen(function* () {
        const test = yield* TestConfect;
        ...
        if (received.outcome !== "received") {
          throw new Error(`expected received outcome, got ${received.outcome}`);
        }
  ...
  it("should adopt a verified eEB inbox item into canonical coverage after a same-quarter card read", async () => {
    const result = await runWithTestConfect(
      Effect.gen(function* () {
        const test = yield* TestConfect;
        ...
        const patient = yield* test.mutation(
          refs.public.patients.createManual,
          {
            patient: {
              ...
              names: [{ family: "Fruehling", given: ["Mara"], prefixes: [] }],
              preferredLanguages: [],
              telecom: [],
            },
            primaryIdentifier: {
    ...
    expect(result.view.view.matchedCoverage?.kostentraegerName).toBe("AOK Neu");
  });
});
